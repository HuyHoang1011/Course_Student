...
export default function CourseStudents() {
  ...
  const handleApproveCourseCompletion = async (studentId) => {
    ...
    try {
      ...
      await axios.post(`http://localhost:5000/api/quizzes/course/${courseId}/approve/${studentId}`, {}, {
  ...
  const handleIssueCertificate = async (studentId) => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      await axios.post(`http://localhost:5000/api/certificates/issue-completed`, {
        courseId: courseId,
        studentId: studentId
      }, {
        headers: { Authorization: `Bearer ${token}` }
  ...
  const handleViewProgress = async (student) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`http://localhost:5000/api/quizzes/course/${courseId}/student/${student.studentId._id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
